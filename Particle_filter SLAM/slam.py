...
class map_t:
    ...
    def grid_cell_from_xy(s, x, y):
        ...
        # #cell_indices = np.zeros((2, l), dtype = int)
        # xa = ((x - s.xmin)/s.resolution).astype(int)
        # ya = ((y - s.ymin)/s.resolution).astype(int)
        # # cell_indices[0,:] = (np.ceil((x - s.xmin) / s.resolution)).astype(np.int16)
        # # cell_indices[1,:] = (np.ceil((y- s.ymin) / s.resolution)).astype(np.int16)
        # # ret = np.vstack((cell_indices[0,:], cell_indices[1,:]))
        # #print(xa.dtype)
        # return xa, ya
...
class slam_t:
    ...
    def stratified_resampling(p, w):
        ...
        # c = w[0]
        # # p_resampled = np.zeros((3, n))
        # # w_resampled = np.ones((n))/n
        # #p_resampled = deepcopy(p)
        # #w_resampled = deepcopy(w)
        # p_resampled = np.zeros_like(p, dtype=np.float64)
        # w_resampled = np.ones_like(w, dtype=np.float64)/n
        # i = 0
        # for m in range(n):
        #     u = r + m/n
        #     while(u > c):
        #         i += 1
        #         c += w[i]
        #     #w_resampled[m] = w_resampled[i]
        #     p_resampled[:,m] = p[:, i]
        
        # return p_resampled, w_resampled
    ...
    def rays2world(s, p, d, head_angle=0, neck_angle=0, angles=None):
        ...
        # the data
        # a  = np.where((d <= s.lidar_dmax) & (d >= s.lidar_dmin))
        # angles = angles[a]
        # d = d[a]
        ...
        # z_lidar = np.zeros(size)
        # homog = np.ones(size)
        # lidar  = np.vstack((x_lidar, y_lidar, z_lidar, homog))

        # # 2. from LiDAR frame to the body frame
        # lidar_to_body = (euler_to_se3(0, head_angle, neck_angle , np.array([0, 0, s.lidar_height])))@lidar

        # # 3. from body frame to world frame
        # body_to_world = (euler_to_se3(0, 0, p[2], np.array([p[0], p[1], s.head_height])))@lidar_to_body

        # return body_to_world[0:2, body_to_world[2,:] > 0.2]
    ...
    def update_weights(w, obs_logp):
        ...
        #### TODO: XXXXXXXXXXX
        #raise NotImplementedError
        # logw = obs_logp + np.log(w)
        # logw_update = logw - slam_t.log_sum_exp(logw)
        # w_new = np.exp(logw_update)
        # return w_new
    ...
    def observation_step(s, t):
        ...
        #### TODO: XXXXXXXXXXX
        #raise NotImplementedError
        # head = s.joint['head_angles'][1][s.find_joint_t_idx_from_lidar(t)]  # head angle at t
        # neck= s.joint['head_angles'][0][s.find_joint_t_idx_from_lidar(t)]
        # cells = np.zeros(s.p.shape[1])
        # for i in range(s.p.shape[1]):
        #     lidar = s.rays2world(s.p[:, i], s.lidar[t]['scan'], head, neck, s.lidar_angles)
        #     x, y = s.map.grid_cell_from_xy(lidar[0,:], lidar[1,:])
        #     cells[i] = np.sum(s.map.cells[x, y])
        
        # s.w = s.update_weights(s.w, cells)

        # largest_w_index = np.argmax(s.w)
        # s.current = s.p[:, largest_w_index]
        # lidar_xy = s.rays2world(s.p[:, largest_w_index], s.lidar[t]['scan'], head, neck, s.lidar_angles)
        # x, y = s.map.grid_cell_from_xy(lidar_xy[0,:], lidar_xy[1,:])
        # s.map.log_odds[x, y] += s.lidar_log_odds_occ 

        # init_x_ind, init_y_ind = s.map.grid_cell_from_xy(s.lidar_dmin, s.lidar_dmin)
        # le = len(x)
        # x_unocc = np.ndarray.flatten(np.linspace([init_x_ind]*le, x, endpoint=False).astype('int64'))
        # y_unocc = np.ndarray.flatten(np.linspace([init_y_ind]*le, y, endpoint=False).astype('int64'))

        # s.map.log_odds[x_unocc, y_unocc] += s.lidar_log_odds_free              # Free cells
        ...
        # s.map.num_obs_per_cell[x, y] += 1
        # s.map.num_obs_per_cell[x_unocc, y_unocc] += 1
